...
SET name        = coalesce(sqlc.narg('name'), name),
    ...
    open_hour   = coalesce(sqlc.narg('open_hour'), open_hour),
    website     = coalesce(sqlc.narg('website'), website),
    is_approved = coalesce(sqlc.narg('is_approved'), is_approved),
    updated_at  = now()
WHERE id = @restaurant_id RETURNING *;
...
WHERE (@follow_by::varchar(64) = '' or urf.user_id = @follow_by)
  AND (@managed_by::varchar(64) = '' or rm.user_id = @managed_by)
  AND (@restaurant_id::varchar(64) = '' or r.id::text = @restaurant_id or r.username = @restaurant_id)
GROUP BY r.id,
         r.name,
         r.description,
         r.avatar_url,
