...
WHERE (sqlc.narg(restaurant_id)::int is null or d.restaurant_id = sqlc.narg(restaurant_id)::int)
  AND (sqlc.narg(cuisine_id)::smallint is null or
       d.cuisine_id in (select id from get_cuisine_recursion_by_id(sqlc.narg(cuisine_id)::smallint)))
  AND (sqlc.narg(search)::text is null or sqlc.narg(search)::text = '' or
       concat(d.name, ' ', r.name) ilike '%' || sqlc.narg(search)::text || '%')
